...
Designed for workflows where you want to work with Atlassian content in your editor or feed it to AI tools, while keeping round-trip fidelity for images, macros, and other rich content.

## Install

```bash
curl -fsSL https://raw.githubusercontent.com/mreider/a-cli/main/install.sh | sh
```

This downloads the latest release for your OS/architecture and installs it to `/usr/local/bin`.

Or download a binary manually from [Releases](https://github.com/mreider/a-cli/releases).
...
```bash
a-cli get PRODUCT-12345
a-cli get PRODUCT-12345 --output-dir ./tickets
```
...
```bash
a-cli push -f ticket.md
a-cli push -f ticket.md --dry-run
```
...
```bash
a-cli apply -f ticket.md
a-cli apply -f ticket.md --dry-run
```
...
a-cli confluence get 85962893 --output-dir ./pages
```
...
```bash
a-cli confluence push -f page.md
a-cli confluence push -f page.md --dry-run
```
